...
class Pelinjatko:
    ...
    def jatka(self, lista_kysymyksia: list):
        for kysymys in range(0, len(lista_kysymyksia)):
            kysymyksentiedot = self.tietokanta.hae_kysymys(lista_kysymyksia[kysymys])
            self.tulosta_kysymys(kysymyksentiedot)
            vastaus = input("Vastaus:")
            self.tallennus(vastaus, lista_kysymyksia, kysymys)
            if vastaus == "quit":
                return
